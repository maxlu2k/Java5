...
<div class="container">
    ...
    <form:form action="/co-so/update/${coSo.id}" method="post" modelAttribute="coSo">
        ...
        <div class="form-group">
            <label for="taiKhoanNganHang">Mã tài khoản ngân hàng:</label>
            <select name="taiKhoanNganHang" class="form-control" id="taiKhoanNganHang">
                <c:forEach items="${danhSachTaiKhoanNganHang}" var="tknh">
                    <option value="${tknh.id}" ${coSo.taiKhoanNganHang.id == tknh.id ? 'selected' : ''}>
                            ${tknh.ma}
                    </option>
                </c:forEach>
            </select>
